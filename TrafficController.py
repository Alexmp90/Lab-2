...
class TrafficController:
    ...
    def __init__(self):
        ...
        self._button = Button(17, "Crossing Button", buttonhandler=None)
        self._display = LCDDisplay(sda=20, scl=21, i2cid=0)
        self._pir = MotionSensor(28)
        self._trafficLight = TFLight()    
    ...
    def stateEntered(self, state):
        ...
        if state == 0:
            ...
            self._trafficLight.NSGreen()
            self._trafficLight.EWRed()
            self._display.showText("Green GO")
        
        elif state == 1:
            # entry actions for state 1
            print('State 1 entered')
            self._timer.start(5)
            self._trafficLight.NSYellow()
            self._trafficLight.EWRed()
            self._display.showText("Caution Yellow")
